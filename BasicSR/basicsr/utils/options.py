...
def yaml_load(f):
    ...
    """
    if os.path.isfile(f):
        # Explicitly open files as UTF-8 to avoid encoding issues on Windows
        with open(f, 'r', encoding='utf-8') as fin:
            return yaml.load(fin, Loader=ordered_yaml()[0])
    else:
...
def copy_opt_file(opt_file, experiments_root):
    ...
    # Use explicit UTF-8 encoding to handle non-ASCII characters on Windows
